...
<generic:shipping command="%= GLOBALS.VIEW %"    
   storeName="%= STORENAME %" 
...
<jutil:element name="form">
   <jutil:attribute name="name">
      <jsp:expression>BillingAddressData.CHANGE</jsp:expression>
   </jutil:attribute>
   <jutil:attribute name="method">
      POST
   </jutil:attribute>   
   <jutil:attribute name="action">
      <jsp:expression>paymentGatewayPageData.SHIPPINGADDRESS</jsp:expression>
   </jutil:attribute>   

<generic:shippingaddress command="%= GLOBALS.VIEW %" 
   storeName="%= STORENAME %" xsl="%= SHIPPINGADDRESSVIEWXSL %" />
   
<xsl:call-template name="submitMenuButtonItem">
   <xsl:with-param name="name">
      BillingAddressData.CHANGE
   </xsl:with-param>
   <xsl:with-param name="command">
      NONE
   </xsl:with-param>
...
<jutil:element name="form">
   <jutil:attribute name="name">
      <jsp:expression>ShippingAddressData.CHANGE</jsp:expression>
   </jutil:attribute>
   <jutil:attribute name="method">
      POST
   </jutil:attribute>   
...
<generic:billingaddress command="%= GLOBALS.VIEW %" 
   storeName="%= STORENAME %" xsl="%= BILLINGADDRESSVIEWXSL %" />
...
<ecommerce:authentication 
   ...
   <jsp:forward page="%= FORWARDLOGOUTPAGE %" />
         ...
         <xsl:variable name="defaultBodyHeading" select="'Step 4 of 6 - Shipping Method Selection'" />
