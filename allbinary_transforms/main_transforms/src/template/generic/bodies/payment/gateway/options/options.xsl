...
<jsp:scriptlet>
//Use request PaymentGateway or
//If only one PaymentGatway option is available then forward to next page
</jsp:scriptlet>
<ecommerce:order storeName="%= STORENAME %" 
   command="%= PaymentGatewayData.NAME.toString() %" >
   <jsp:scriptlet>
      final String forwardPage = basicWeblisketSession.getPaymentMethod() + 
         paymentGatewayPageData.STARTCHECKOUT;
   </jsp:scriptlet>
...
Select Payment Method:<p></p>
<payment:gatewayViewExisting
   command="%= GLOBALS.VIEW %"
   xsl="%= PAYMENTGATEWAYOPTIONSXSL %" />
...
<ecommerce:authentication 
   ...
   <jsp:forward page="%= FORWARDLOGOUTPAGE %" />
         ...
         <xsl:variable name="defaultBodyHeading" select="'Payment Options'" />
