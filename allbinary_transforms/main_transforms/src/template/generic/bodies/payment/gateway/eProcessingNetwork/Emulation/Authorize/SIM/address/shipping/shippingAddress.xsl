...
<jutil:element name="form">
   <jutil:attribute name="name">
      <jsp:expression>CONTINUE</jsp:expression>
   </jutil:attribute>
   <jutil:attribute name="method">
      POST
   </jutil:attribute>   
   <jutil:attribute name="action">
      <jsp:expression>paymentGatewayPageData.SHIPPINGADDRESSACTION</jsp:expression>
   </jutil:attribute>      

   <generic:taxaddress 
      command="%= GLOBALS.VIEW %" logic="false"
      storeName="%= STORENAME %" xsl="%= EDITTAXADDRESSXSL %" >

      <generic:taxaddress 
         command="%= GLOBALS.NEW %"
         storeName="%= STORENAME %" xsl="%= NEWTAXADDRESSXSL %" >
      </generic:taxaddress>
...
<ecommerce:authentication 
   ...
   <jsp:forward page="%= FORWARDLOGOUTPAGE %" />
         ...
         <xsl:variable name="defaultBodyHeading" select="'Step 1 of 4 - Shipping/Tax Location Selection'" />
