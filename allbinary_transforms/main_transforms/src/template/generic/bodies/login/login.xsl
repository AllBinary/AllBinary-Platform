...
-->
   ...
   <xsl:import href="/template/generic/bodies/imports/login/loginForm.xsl" />
   <xsl:import href="/template/generic/bodies/imports/register/registerForm.xsl" />
...
<jsp:scriptlet> 
 ...
 String forwardName = request.getParameter(ForwardData.NAME);
 String forwardValue = request.getParameter(ForwardData.VALUE);
 
 if(StringValidationUtil.isEmpty(forwardPage))
    forwardPage = paymentGatewayPageData.PAYMENTOPTIONS;
    
 if(StringValidationUtil.isEmpty(forwardName))
    forwardName = GLOBALS.CHECKOUT;
 if(StringValidationUtil.isEmpty(forwardValue))
    forwardValue = GLOBALS.CHECKOUT;
</jsp:scriptlet>

<xsl:call-template name="registerForm" >
   <xsl:with-param name="xslForwardPage" >forwardPage</xsl:with-param>
</xsl:call-template>
...
<xsl:call-template name="loginForm" >
   <xsl:with-param name="xslForwardPage" >forwardPage</xsl:with-param>
   <xsl:with-param name="xslForwardName" >forwardName</xsl:with-param>
   <xsl:with-param name="xslForwardValue" >forwardValue</xsl:with-param>
</xsl:call-template>
 ...
 */
         ...
         <xsl:variable name="defaultBodyHeading" select="'Customer Login'" />
