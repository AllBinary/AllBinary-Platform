...
if(command!=null)
{
   if(command.compareTo(CANCELORDER)==0)
   {
</jsp:scriptlet>
<generic:orderhistory    
   command="%= OrderHistoryData.SETSTATUS  %"       
   status="%= OrderHistoryData.CANCELLED %" 
   storeName="%= STORENAME %"
   xsl="%= ORDERHISTORYXSL %" />
<br/>
<generic:orderhistory    
...
<generic:order command="%= command %" 
   storeName="%= STORENAME %" xsl="%= REVIEWORDERVIEWXSL %"/>

<jutil:element name="form">
   <jutil:attribute name="name">
      <jsp:expression>CANCELORDER</jsp:expression>
   </jutil:attribute>
   <jutil:attribute name="method">
      POST
   </jutil:attribute>   
   <jutil:attribute name="action">
      <jsp:expression>REVIEWPAGE</jsp:expression>
   </jutil:attribute>   
<p/>
   ...
   <jutil:element name="input">
      ...
      <jutil:attribute name="name">
         <jsp:expression>OrderHistoryData.CANCELLED</jsp:expression>
      </jutil:attribute>
      <jutil:attribute name="value">
         <jsp:expression>OrderHistoryData.STATUS</jsp:expression>
      </jutil:attribute>
   </jutil:element>
   ...
   <jutil:element name="input">
      ...
      <jutil:attribute name="name">
         <jsp:expression>OrderData.ID</jsp:expression>
      </jutil:attribute>
      <jutil:attribute name="value">
         <jsp:expression>request.getParameter(OrderData.ID)</jsp:expression>
      </jutil:attribute>
   </jutil:element> 
...
<ecommerce:authentication 
   ...
   <xsl:call-template name="jspForward">
      ...
      <xsl:with-param name="FORWARDVALUE">
         ...
         <xsl:variable name="defaultBodyHeading" select="'Order Review'" />
