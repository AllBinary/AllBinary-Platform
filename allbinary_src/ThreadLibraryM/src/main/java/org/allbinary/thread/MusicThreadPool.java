...
public class MusicThreadPool extends ThreadPool
{
    ...
    {
        logUtil.put(new StringMaker().append(this.threadPoolStrings.START_TASK).append(StringUtil.getInstance().toString(task)).append(System.currentTimeMillis()).toString(), this, commonStrings.RUN);
    }

    protected void completedTask(Runnable task)
    {
        logUtil.put(new StringMaker().append(this.threadPoolStrings.COMPLETE_TASK).append(StringUtil.getInstance().toString(task)).append(System.currentTimeMillis()).toString(), this, commonStrings.RUN);
    }
