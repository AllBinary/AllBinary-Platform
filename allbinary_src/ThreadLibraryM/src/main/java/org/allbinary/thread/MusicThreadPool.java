...
public class MusicThreadPool extends ThreadPool
{
    ...
    {
        logUtil.put(new StringMaker().append(this.threadPoolStrings.START_TASK).append(StringUtil.getInstance().toString(task)).appendlong(System.currentTimeMillis()).toString(), this, commonStrings.RUN);
    }

    @Override
    protected void completedTask(Runnable task)
    {
        logUtil.put(new StringMaker().append(this.threadPoolStrings.COMPLETE_TASK).append(StringUtil.getInstance().toString(task)).appendlong(System.currentTimeMillis()).toString(), this, commonStrings.RUN);
    }
